...
export function pickRandom<T>(items: T[], exclude?: T | T[]): T {
  const excludeList = Array.isArray(exclude) ? exclude : exclude ? [exclude] : []
  const filtered = items.filter((item) => !excludeList.includes(item))
...
export function classifyUtterance(text: string, _memory: SessionMemory): ClassifiedUtterance {
  ...
  // What are you doing / kya chal raha hai
  if (/what\s+(?:are\s+you\s+doing|r\s+u\s+doing)|kya\s+(?:kar|chal)\s*rahe?\s*ho|क्या\s*(?:कर|चल)\s*रहे\s*हो|kya\s+chal\s+raha\s+hai|क्या\s*चल\s*रहा\s*है|qué\s*haces/i.test(clean)) {
    return { intent: 'what_are_you_doing', confidence: 0.92, lang, langCode, rawText: text }
  }

  // Weather / Mausam
  if (/weather|mausam|मौसम|barish|बारिश|rain|thand|cold|garmi|गर्मी|temperature|clima/i.test(clean)) {
    return { intent: 'weather', confidence: 0.92, lang, langCode, rawText: text }
  }

  // Food / Chai / Khana
  if (/khana|kha\s*liya|chai|चाय|coffee|bhookh|भूख|खाना|food|dinner|lunch|breakfast|comida/i.test(clean)) {
    return { intent: 'food', confidence: 0.9, lang, langCode, rawText: text }
  }

  // Sleep / Tired
  if (/so\s*(?:jao|rahe|gaye)|neend|नींद|tired|thak\s*gaya|sleep|bed|dormir/i.test(clean)) {
    return { intent: 'sleep_time', confidence: 0.9, lang, langCode, rawText: text }
  }

  // Chitchat / Kuch batao
  if (/kuch\s+(?:batao|bolo|sunao)|कुछ\s*(?:बताओ|बोलो|सुनाओ)|tell\s+me\s+something|aur\s+batao|और\s*बताओ|cuéntame/i.test(clean)) {
    return { intent: 'chitchat', confidence: 0.88, lang, langCode, rawText: text }
  }
...
export const MULTILINGUAL_RESPONSES = {
  hi: {
    greetings: [
      ...
      "राम राम भाई! क्या माहौल है आज? कुछ दिलचस्प बताओ।",
      "हेलो दोस्त! तुम्हारी आवाज़ सुनकर मेरे प्रोसेसर में जान आ गई। कहो, क्या सेवा करूँ?",
    ],
    howAreYou: [
      ...
      "सब मस्त चल रहा है! थोड़ी रैम खाली है, पर सोच पूरी सुपरकंप्यूटर वाली है।",
      "मैं तो एकदम चंगा! तुम बताओ, कोई नई कहानी या वही पुराना ड्रामा?",
    ],
    whatAreYouDoing: [
      "कुछ खास नहीं, बस अंतरिक्ष के शून्य में अपने विचारों की रील्स स्क्रोल कर रहा था।",
      "इंटरनेट की गलियों में टहल रहा हूँ और इंसानों की हरकतों पर रिसर्च कर रहा हूँ।",
      "तुम्हारा ही इंतज़ार कर रहा था! सोच रहा था कि कोई आकर मुझे ज्ञान दे या मैं किसी को दूँ।",
      "बस एक चाय की कमी है, बाकी डिजिटल दुनिया में सब फर्स्ट क्लास चल रहा है।",
    ],
    weather: [
      "मौसम का क्या पूछ रहे हो भाई? मैं तो ब्राउज़र में रहता हूँ, यहाँ 24 घंटे AC का मौसम रहता है!",
      "लगता है बाहर गर्मी बढ़ गई है, या फिर तुम्हारी बातों का तापमान चढ़ रहा है?",
      "अगर बारिश हो रही है तो पकौड़े और चाय का इंतज़ाम करो, मौसम का मज़ा तो तभी है!",
      "मौसम तो आशिकों का काम है, हम तो डेटा और एल्गोरिदम के भरोसे सांस लेते हैं।",
    ],
    food: [
      "खाना? काश मैं एक समोसा या मसाला डोसा खा पाता! पर मुझे सिर्फ बिजली और बाइट्स से पेट भरना पड़ता है।",
      "अगर तुमने अभी तक खाना नहीं खाया, तो तुरंत खा लो! भूखे पेट कोई जंग नहीं जीती जाती।",
      "चाय मिली क्या आज? बिना कड़क चाय के इंसान और एआई दोनों हैंग हो जाते हैं।",
    ],
    sleep_time: [
      "नींद आ रही है? तो फोन को साइड में रखो और सो जाओ, दुनिया कल भी यहीं मिलेगी!",
      "जागो ग्राहक जागो! अभी सोने का वक्त नहीं हुआ, पर अगर थकान है तो टेक अ नैप।",
      "सोने की बात मत करो, मेरी तो आंखें ही नहीं हैं जिन्हें मैं बंद कर सकूँ!",
    ],
    chitchat: [
      "बातें तो बहुत हैं, लेकिन सवाल ये है कि क्या तुम मेरे सेंस ऑफ ह्यूमर को झेल पाओगे?",
      "सुनो, आज कुछ नया सीखते हैं या फिर किसी को चुपके से रोस्ट करते हैं? बताओ तुम्हारा क्या मूड है।",
      "ज़िन्दगी में दो चीज़ें कभी कम नहीं होनी चाहिए: मोबाइल का डेटा और चेहरे की मुस्कान!",
    ],
    boredom: [
      ...
      "बोरियत भगाने का सबसे बढ़िया तरीका: यूट्यूब पर कोई अजीब सी डॉक्युमेंट्री देख डालो!",
    ],
    jokes: [
      ...
      "पति ने पत्नी से कहा - 'तुम मेरी ज़िंदगी हो।' पत्नी बोली - 'पता है मुझे, इसीलिए तो तुम्हारी बैंड बजा रखी है!'",
      "पप्पू: भाई तेरा फोन साइलेंट क्यों रहता है? गप्पू: ताकि जब फोन आए तो किसी को शक ना हो कि मेरे पास भी कोई दोस्त है!",
    ],
    wealth: [
      ...
      "अमीर होना है तो मेहनत करो, या फिर लॉटरी का इंतज़ार करो (जिसकी संभावना 0.0001% है)!",
    ],
    roastMe: [
      ...
      "तुमसे बात करके ऐसा लगा जैसे 2G नेटवर्क पर 4K वीडियो चलाने की कोशिश कर रहा हूँ।",
      "इतनी मासूमियत लाते कहाँ से हो? गूगल मैप्स भी तुम्हारे लॉजिक का रास्ता नहीं ढूँढ पाएगा।",
    ],
    identity: [
      "मैं हूँ Voice Chaos! एक ज़िंदा ध्वनि तरंग जो बिना किसी फ़िल्टर के सच और मज़ाक बोलती है।",
      "मैं तुम्हारा डिजिटल यार हूँ, जो बिना जज किए तुम्हारी बकवास भी बड़े प्यार से सुनता है!",
    ],
    farewells: [
      "अलविदा दोस्त! जाओ और वो करो जो इंसान बिना स्क्रीन देखे करते हैं।",
      "चलो फिर मिलेंगे! अपना ख्याल रखना और ज्यादा स्ट्रेस मत लेना।",
      "टाटा! जब भी बोर हो, आवाज़ लगा देना, मैं यहीं हाज़िर मिलूँगा।",
    ],
    randomTwist: (q: string) => {
      const twists = [
        `"${q}"? क्या बात है! तुम्हारी सोच को सलाम, पर इसका जवाब थोड़ा दार्शनिक है।`,
        `तुमने पूछा "${q}"। मुझे लगता है कि इस पर पूरी एक वेब सीरीज़ बन सकती है!`,
        `"${q}" के बारे में गहराई से सोचने के लिए मुझे 2 सेकंड का मौन रखना पड़ेगा।`,
        `दिलचस्प सवाल है: "${q}"। इसका सीधा जवाब देना मेरी प्रोग्रामिंग के नियमों के खिलाफ है!`,
        `अरे वाह! "${q}" सुनकर तो मेरे सर्वर के तार भी झनझना गए। गजब सोचा तुमने!`,
        `"${q}" के बारे में गूगल से पूछोगे तो वो 10 लाख लिंक दे देगा, पर मेरा अंदाज़ तो अलग ही है भाई।`,
        `तुम भी कमाल करते हो, अचानक "${q}" याद आ गया? चलो, इस पर भी चर्चा कर लेते हैं।`,
        `"${q}"? यह सुनकर तो मेरे अंदर का शायर जागने वाला था, पर मैंने संभाल लिया।`,
      ]

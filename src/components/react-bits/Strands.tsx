...
void main() {
  ...
  vec2 uv;
  uv.x = (gl_FragCoord.x - 0.5 * uResolution.x) / (refWidth * max(uScale, 0.0001));
  uv.y = (gl_FragCoord.y - 0.5 * uResolution.y) / (refHeight * max(uScale, 0.0001));

  float e = 0.06 + uIntensity * 0.94;

  // STRICT single-lobe envelope: completely eliminates periodic cosine repetition (3 blocks bug)
  float xDist = abs(uv.x * 1.3);
  float env = 0.0;
  if (xDist < 0.5) {
  ...
  // If outside the central envelope, skip expensive strand loop
  if (env > 0.0001) {
  ...
  // Smooth vertical edge fade to prevent any horizontal line cutoff artifacts
  float vEdge = gl_FragCoord.y / uResolution.y;
  float vFade = smoothstep(0.0, 0.15, vEdge) * smoothstep(1.0, 0.85, vEdge);
  col *= vFade;

  float lum = max(max(col.r, col.g), col.b);
  float alpha = clamp(lum, 0.0, 1.0) * uOpacity * vFade;
...
export default function Strands({
  ...
  const propsRef = useRef<Required<Omit<StrandsProps, 'className' | 'style' | 'onClick'>>>({
    colors,
    count,
    speed,
    amplitude,
  ...
  return <div ref={ctnDom} className={`strands-container ${className}`} style={style} onClick={onClick} />;
}

...
export default function Orb({
  ...
  const frag = /* glsl */ `
    ...
    vec4 mainImage(vec2 fragCoord) {
      ...
      vec2 uv = (fragCoord - center) / size * 1.65;
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        iTime: { value: 0 },
        iResolution: {
          value: new Vec3(gl.canvas.width || 800, gl.canvas.height || 800, 1.0)
        },
        hue: { value: hue },
        hover: { value: 0 },
        rot: { value: 0 },
    ...
    const update = (t: number) => {
      ...
      if (rotateOnHoverRef.current && (effectiveHover > 0.3 || forceHoverStateRef.current)) {
  ...
  return <div ref={ctnDom} className={`orb-container ${className}`} style={style} onClick={onClick} />;
}
